<?php
	...
	//place
		...
		function deletePlace($link,$id){
			$sql = 'UPDATE place SET visibility = 0 WHERE id ='.$id;
			if (mysqli_query($link, $sql)) {
				//success
				return "success";
			}else{
		...
		function updateLocation($link,$id,$location)
		{
			$sql = 'UPDATE place SET location="'.$location.'" WHERE id='.$id;
			if (mysqli_query($link, $sql)) {
				//success
				return "success";
			}else{
		...
		function getPlaceFromDayLife($link,$days)
		{
			...
			if($result = mysqli_query($link, $sql)){
				...
				if($num_rows == 0){
					...
					while($r = mysqli_fetch_assoc($result)) {
						$r['photo'] = 'file_upload/place/'.$r['name'].'-'.$r['location'].'/'.$r['photo'];
						$r['type'] = $this->getPlaceType($link, $r['id']);
						$rows[] = $r;
					}
					return $rows;
				}
		...
		function getPlaceType($link,$place_id)
		{
			$isTop = false;
			$isTrending = false;
			$isNew = false;
			
			$sql = 'SELECT type FROM recommendation WHERE place_id="'.$place_id.'"';
			if($result = mysqli_query($link, $sql)){
				$num_rows = mysqli_num_rows($result);
	...
	//place_category
		...
		function advanceSearch($link,$category,$city,$nearby,$x,$y,$offset)
		{
			$first = true;
			$sql = "";
			foreach($category as $feature){
				if($first)
				{
					$sql = 'SELECT place_id FROM place_category WHERE category="feature" AND value="'.$feature.'"';
					$first = false;
				}
				else
				{
					$sql .= ' UNION SELECT place_id FROM place_category WHERE category="feature" AND value="'.$feature.'"';
					
				}
			}
			
			return $this->getPlaceByCategory($link,$city,$nearby,$x,$y,$sql,$offset);
		}
		
		//distinct (select union select....)
		function getPlaceByCategory($link,$city,$nearby,$x,$y,$sql,$offset){
			if($sql == "")
			{
				return '{"status":"error","message":"place not found"}';
			}
			else
			{
				if($sql == ""){
					...
					$query = 'SELECT DISTINCT place_id FROM ('.$sql.') as tabel ';//LIMIT '.$offset.',20'; 
				}
				//return $query;
				if($result = mysqli_query($link, $query)){
					...
					if($num_rows == 0){
						...
						$rows = array();
						while($r = mysqli_fetch_assoc($result)) {
							$sql = 'SELECT * FROM place WHERE city LIKE "%'.$city.'%" AND visibility = 1 AND id ='.$r['place_id'];
							if($result2 = mysqli_query($link, $sql))
							{
								...
								if($num_rows != 0){
									...
									if($nearby == 'true'){
										$pieces = explode(",", $r2['location']);
										$pieces[0] = str_replace('(', '', $pieces[0]);
										$pieces[1] = str_replace(')', '', $pieces[1]);
										if(abs(floatval($pieces[0]) - floatval($x)) <= 0.00075 && abs(floatval($pieces[1]) - floatval($y)) <= 0.00075)
										{
											$r2['photo'] = 'file_upload/place/'.$r2['name'].'-'.$r2['location'].'/'.$r2['photo'];
											$rows[] = $r2;
										}
									}else{
										$r2['photo'] = 'file_upload/place/'.$r2['name'].'-'.$r2['location'].'/'.$r2['photo'];
	...
	//review
		function addReview($link,$review)
		{
			...
			if (mysqli_query($link, $sql)) {
				//success
				$this->countRating($link,$place_id);
				return "success";
			}else{
				//error
		...
		function editReview($link,$id,$review)
		{
			...
			if (mysqli_query($link, $sql)) {
				//success
					$sql2 = 'SELECT place_id FROM review WHERE id="'.$id.'" LIMIT 1';
					if($result2 = mysqli_query($link,$sql2))
					{
						$value = mysqli_fetch_assoc($result2);
						$this->countRating($link,$value['place_id']);
					}
		...
		{
			$sql = 'SELECT * FROM review WHERE place_id="'.$place_id.'" ORDER BY date DESC';
		...
		{
			$sql = 'SELECT * FROM review WHERE profile_id="'.$profile_id.'" ORDER BY date DESC';
	...
	//recommendation
		function getNewPlace($link)
		{
			...
			$sql = 'SELECT * FROM place WHERE visibility = 1 ORDER BY create_time DESC LIMIT 15';			
		...
		{
			$sql = 'SELECT ranking FROM recommendation WHERE place_id="'.$place_id.'" AND type="top"';
