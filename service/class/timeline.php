<?php
	...
	//post
		...
		{
			$sql = 'SELECT * FROM post WHERE profile_id ='.$profile_id.' ORDER BY time DESC LIMIT 50';
		...
		{
			$sql = 'SELECT * FROM (SELECT * FROM post WHERE profile_id = ANY(SELECT profile_id FROM follower WHERE follower_id='.$profile_id.')
					UNION SELECT * FROM post WHERE profile_id ='.$profile_id.') as tab ORDER BY time DESC LIMIT 50';
