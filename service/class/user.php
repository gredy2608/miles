<?php
	...
	//follower
			...
			if($result = mysqli_query($link, $sql)){
				...
				if($num_rows == 0){
					...
					while($r = mysqli_fetch_assoc($result)) {
						...
						$r['photo'] = $value->photo;
						$r['follow'] = $this->checkFollow($link,$r['follower_id'],$profile_id);
						$rows[] = $r;
					}
					return $rows;
				}
		...
		function getSuggestFollow($link,$profile_id)
		{
			$sql = 'SELECT
				...
				WHERE
				  me.id = '.$profile_id.' AND their_friends.profile_id <> '.$profile_id.'
				GROUP BY
				  me.id,
			...
			if($result = mysqli_query($link, $sql)){
				$num_rows = mysqli_num_rows($result);
				if($num_rows == 0){
					return '{"status":"error","message":"suggest follow not found"}';
				}else{
					$rows = array();
					while($r = mysqli_fetch_assoc($result)) {
						$sql = 'SELECT last_name,first_name,photo FROM profile WHERE id="'.$r['suggested_friend_id'].'" LIMIT 1';
						$result2 = mysqli_query($link,$sql);
						$value = mysqli_fetch_object($result2);
						$r['name'] = $value->first_name.' '.$value->last_name;
						$r['photo'] = $value->photo;
		...
		function checkFollow($link,$profile_id,$follower_id)
		{
	...
	//preference
		...
		function deletePreferenceAll($link,$profile_id)
		{
			$profile_id = mysqli_escape_string($link,$profile_id);
			
			$sql = 'DELETE FROM preference WHERE profile_id ='.$profile_id;
			if (mysqli_query($link, $sql)) {
				//success
				return 'success';
			}else{
		...
		function deletePreference($link,$profile_id,$value)
		{
			...
			$sql = 'DELETE FROM preference WHERE profile_id ='.$profile_id.' AND value='.$value;
			if (mysqli_query($link, $sql)) {
				//success
				return '{"status":"success"}';
			}else{
	...
	//profile
		...
		{
			$sql = 'UPDATE profile SET photo="'.$newphoto.'" WHERE id='.$id;
		...
		function updateProfile($link,$id,$user)
		{
			$fname = mysqli_escape_string($link,$user['first_name']);
			$lname = mysqli_escape_string($link,$user['last_name']);
			$birthday = mysqli_escape_string($link,$user['birthday']);
			...
			$sql = 'UPDATE profile SET first_name="'.$fname.'",last_name="'.$lname.'",birthday="'.$birthday.'",sex="'.$sex.'",phone_number="'.$phone.'" WHERE id='.$id;
			if (mysqli_query($link, $sql)) {
				//success
				return '{"status":"success"}';
			}else{
