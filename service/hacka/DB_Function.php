<?php
	...
	class DB_Functions
	{
		 ...
		 */
		public function storeUser($link,$gcm_regid) 
		{
			...
			{*/
				$query = 'INSERT INTO gcm_users_bpjs(gcm_regid) VALUES("'.$gcm_regid.'")';
				$result = mysqli_query($link,$query);
				// check for successful store
				if ($result) {
					// get user details
					$id = mysqli_insert_id($link); // last inserted id
					$result = mysqli_query($link,"SELECT * FROM gcm_users_bpjs WHERE id = $id") or die(mysql_error());
					// return user details
					if (mysqli_num_rows($result) > 0) {
						echo mysqli_fetch_array($result);
					} else {
		 ...
		 * returns user details
		 */
		public function deleteUser($link, $gcm_regid) 
		{
			// insert user into database
			$query = 'DELETE FROM gcm_users_bpjs ';
			$result = mysqli_query($link,$query);
			// check for successful store
			if ($result) {
				return true;
